...
class RiotWatcher:
	def __init__(self, key, default_region=NORTH_AMERICA, limits=(RateLimit(10, 10), RateLimit(500, 600)), enforce_limits=True):
	...
	def base_request(self, url, region, static=False, **kwargs):
		...
		requestStr = 	'https://{proxy}.api.pvp.net/api/lol/{static}{region}/{url}'.format(
		...
		r = requests.get( requestStr, params=args )
		if not static:
			for lim in self.limits:
				lim.add_request()
		raise_status(r)
