        ...
        try:
            if (user_Id is not None):
                ...
                if (min_id is not None and max_id is not None):
                    ...
                    currentTweet = tweepy.Cursor(self.api.user_timeline, trim_user=True, user_id=userId, count=count,
                                                 contributor_details=False, since_id=max_id).items()

                elif (min_id is None and max_id is None):
                    if (self.loud):
                        print "Storing for new user!"
                    currentTweet = tweepy.Cursor(self.api.user_timeline, trim_user=True, user_id=userId, count=count,
                                                 contributor_details=False).items()
